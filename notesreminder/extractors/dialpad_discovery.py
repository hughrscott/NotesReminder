...
from notesreminder.extractors.dialpad_targets import (  # noqa: E402
...
def select_target_candidates(
    ...
    window_days=7,
    limit=25,
    candidate_source="lead-attention",
    start_date=None,
    end_date=None,
):
    ensure_lead_followup_schema(conn)
    conn.row_factory = sqlite3.Row
    if candidate_source == "hubspot-contacts":
...
def search_target(page, target, per_target_limit):
    ...
    for path_name, url in SEARCH_ROUTES:
        ...
        try:
            ...
            if (
                path_name == "conversation_history"
                and filter_diagnostics.get("expected_school_scope")
                ...
                path_results.append(
                    {
                        ...
                        "visible_target_phone": False,
                        "call_review_url_count": 0,
                        "filter_diagnostics": filter_diagnostics,
                    }
                )
                best["outcome"] = "school_scope_mismatch"
                break
            if path_name == "conversation_history":
    ...
    elif best["outcome"] == "school_scope_mismatch":
        pass
...
def target_search_summary(conn, run_id=None):
    ...
    blocked_count = outcomes["ui_blocked"] + outcomes["filter_not_supported"] + outcomes["auth_blocked"] + outcomes["school_scope_mismatch"]
    return {
        "run_id": run_id,
