...
def build_reports(db_path, school, limit, start_date=None, end_date=None):
    conn = sqlite3.connect(db_path)
    conn.row_factory = sqlite3.Row
    ensure_lead_followup_schema(conn)
    try:
        refresh_identity_matches(conn)
        report = build_gap_report(conn, school, limit, start_date=start_date, end_date=end_date)
        conn.commit()
    finally:
        conn.close()
...
def main():
    ...
    _, markdown, json_text = build_reports(args.db, args.school, args.limit, args.start_date, args.end_date)
